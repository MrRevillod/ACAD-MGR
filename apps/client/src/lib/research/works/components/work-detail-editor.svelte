<script lang="ts">
	...
	import { useQueryClient } from "$shared/http/tanstack"
	...
	import WorkAuthorsList from "./work-authors-list.svelte"
	import WorkEditForm from "./work-edit-form.svelte"
	...
	let { workId, token, onBack }: Props = $props()

	const query = useWorkDetailQuery(() => workId)

	const qc = useQueryClient()

	let viewing = $state(true)
...
<div
	class="flex h-[calc(100dvh-7.5rem)] flex-col overflow-hidden rounded-xl border border-corp-gray/20 bg-white"
>
					...
					<article>
						<header>
							<div class="flex flex-wrap items-center gap-2">
								{#if onBack}
									...
									<a
										...
										class="inline-flex items-center gap-1 text-sm font-medium text-corp-blue transition-colors hover:text-corp-blue/80"
									>
										<ArrowLeft class="size-3.5" />
										Volver
									</a>
									<span class="h-3 w-px bg-corp-gray/20"></span>
								{/if}
							...
							<div class="mt-3 flex items-start justify-between gap-3">
								<h1
									class="min-w-0 flex-1 text-2xl font-semibold text-balance text-[#1A1A1A]"
								>
								...
								<button
									...
									class="mt-0.5 inline-flex shrink-0 items-center gap-1.5 rounded-lg border border-corp-gray/20 px-3 py-1.5 text-sm font-medium text-corp-gray transition-colors hover:border-corp-blue/30 hover:text-corp-blue active:scale-[0.96] focus-visible:ring-2 focus-visible:ring-corp-blue/30"
								>
									<Pencil class="size-4" />
									Editar
								</button>
						...
						<div
							class="mt-8 lg:grid lg:grid-cols-[minmax(0,1fr)_20rem] lg:items-start lg:gap-10"
						>
										...
										<div class="space-y-1">
											{#each work.topics as t (t.topicId)}
												<div>
													<button
														type="button"
														class="flex w-full items-center gap-2 rounded-md px-1 py-0.5 text-left transition-colors hover:bg-corp-gray/5 focus-visible:ring-2 focus-visible:ring-corp-blue/30"
														onclick={() => toggleTopic(t.topicId)}
														aria-expanded={expandedTopicId ===
															t.topicId}
													>
														<span
	...
	{#if query.data && !viewing}
		<div class="shrink-0 border-t border-corp-gray/10 bg-white px-5 py-3">
			<div class="flex flex-wrap items-center justify-end gap-2">
				<button
					type="button"
					onclick={() => (showRestoreConfirm = true)}
					disabled={isSaving}
					class="inline-flex min-h-10 items-center gap-1.5 rounded-lg border border-red-200 px-3 py-1.5 text-sm font-medium text-red-600 transition-colors hover:border-red-300 hover:bg-red-50 active:scale-[0.96] focus-visible:ring-2 focus-visible:ring-red-600/30 disabled:pointer-events-none disabled:opacity-50"
				>
				...
				<button
					...
					disabled={isSaving}
					class="inline-flex min-h-10 items-center gap-1.5 rounded-lg border border-corp-gray/20 px-3 py-1.5 text-sm font-medium text-corp-gray transition-colors hover:border-corp-gray/40 hover:text-[#1A1A1A] active:scale-[0.96] focus-visible:ring-2 focus-visible:ring-corp-gray/20 disabled:pointer-events-none disabled:opacity-50"
				>
					<X class="size-4" />
					Cancelar
				</button>
				<button
					...
					onclick={() => editSubmit?.()}
					disabled={isSaving}
					class="inline-flex min-h-10 items-center gap-1.5 rounded-lg bg-corp-blue px-3 py-1.5 text-sm font-medium text-white transition-colors hover:bg-corp-blue/90 active:scale-[0.96] focus-visible:ring-2 focus-visible:ring-corp-blue/30 disabled:pointer-events-none disabled:opacity-60"
				>
					<Check class="size-4" />
					{isSaving ? "Guardando…" : "Guardar y volver al perfil"}
				</button>
			</div>
		</div>
	{/if}
...
>
	...
	<div class="mt-5 flex justify-end gap-2">
		<button
			...
			class="inline-flex items-center rounded-lg border border-corp-gray/20 px-3 py-1.5 text-sm font-medium text-corp-gray transition-colors hover:border-corp-gray/40 hover:text-[#1A1A1A] active:scale-[0.96] focus-visible:ring-2 focus-visible:ring-corp-gray/20"
		>
			Cancelar
		</button>
		<button
			...
			class="inline-flex items-center gap-1.5 rounded-lg bg-red-600 px-3 py-1.5 text-sm font-medium text-white transition-colors hover:bg-red-700 active:scale-[0.96] focus-visible:ring-2 focus-visible:ring-red-600/30"
		>
			<RotateCcw class="size-4" />
			Restaurar
		</button>
