	...
	<table class="w-full caption-bottom text-sm">
		<thead>
			<tr class="border-b border-corp-gray/20 bg-gray-100 text-left">
				<th class="px-4 py-3 text-xs font-medium tracking-wide uppercase text-corp-gray"
					>Nombre</th
				>
				<th
					class="px-4 py-3 text-right text-xs font-medium tracking-wide uppercase text-corp-gray"
					>Total</th
				>
				<th
					class="px-3 py-3 text-right text-xs font-medium tracking-wide uppercase text-corp-gray"
					>Scopus</th
				>
				<th
					class="px-3 py-3 text-right text-xs font-medium tracking-wide uppercase text-corp-gray"
		...
		<tbody>
			{#each publishers as p, i (p.academicId)}
				<tr
					class="border-b border-corp-gray/10 transition-colors {i % 2 === 0
						? 'bg-gray-50'
						: ''} hover:bg-corp-blue/4"
				>
					<td class="px-4 py-2.5">
						<button
							class="text-left text-[#1A1A1A] font-medium hover:text-corp-blue hover:underline"
							onclick={() => void goto(`/academics/${p.academicId}`)}
						>
